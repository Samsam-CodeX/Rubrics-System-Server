async function loadForm() {
    ...
    const targetStudentId = Number(params.get('studentId'));
    ...
    panel.innerHTML = `
            <a href="student_GroupInfo?group=${groupNumber}">
                ← Back to evalutaion
            </a>
    `;

    if (!targetStudentId || !groupNumber) {
        document.body.innerHTML = '<h1>Invalid rating request.</h1>';
        return;
    }

    try {
        const response = await fetch(`/api/ratable-students/${targetStudentId}`);
        if (!response.ok) {
    ...
    form.addEventListener('submit', async event => {
        ...
        try {
            ...
            setTimeout(() => {
                window.location.href = `student_GroupInfo?group=${groupNumber}`;
            }, 1200);
