async function load() {
    ...
    panel.innerHTML = `
        <a href="studentDashboard?studentId=${raterId}">
            ← Back to dashboard
        </a>
    `;
    ...
    try {
        ...
        infoText.innerHTML = `<strong>Group ${group.groupNumber}</strong> — choose a student to rate.`;
        ...
        group.members.forEach(member => {
            ...
            if (!buttonDisabled) {
                button.addEventListener('click', () => {
                    window.location.href = `ratingForm?studentId=${member.studentId}&group=${group.groupNumber}&raterId=${raterId}`;
                });
            }
