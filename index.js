    ...
    try {
        ...
        try {
            ...
            await connection.query(
                'DELETE FROM ratings WHERE student_id = ? OR rater_id = ?;',
                [studentId, studentId]
            );
...
app.get('/api/ratable-students/:id', requireStudent, async (req, res) => {

    const targetStudentId = Number(req.params.id);
    const currentStudentId = req.session.studentId;

    try {

        const [me] = await pool.query(
            'SELECT groupNumber FROM students WHERE studentId = ?',
            [currentStudentId]
        );
        ...
        const myGroup = me[0].groupNumber;
        ...
        const [rows] = await pool.query(`
            ...
            WHERE studentId = ?
              AND studentId <> ?
              AND groupNumber <> ?;
        `, [targetStudentId, currentStudentId, myGroup]);
...
app.get('/api/students/:id/ratings', requireTeacher, async (req, res) => {
...
app.get('/api/groups/:groupNumber', requireStudent, async (req, res) => {
    try {
        const groupNumber = Number(req.params.groupNumber);
        const raterId = req.session.studentId;
        ...
        const [students] = await pool.query(`
            SELECT
                ...
                s.groupNumber,
                ROUND(averages.studentAverage, 2) AS studentAverage,
                COALESCE(averages.totalRaters, 0) AS totalRaters,
        ...
        const members = students
            ...
            .map(student => ({
                ...
                totalRaters: student.totalRaters,
                ratedByMe: student.ratedByMe
            }));

        const average = groupAverage.find(avg => avg.groupNumber === groupNumber);

        const response = {
            currentStudentId: req.session.studentId,
            groupNumber: group.groupNumber,
            totalStudents: group.totalStudents,
...
app.get('/api/admin/groups/:groupNumber', requireTeacher, async (req, res) => {
    try {
        const groupNumber = Number(req.params.groupNumber);
...
app.get('/api/dashboard', requireTeacher, async (req, res) => {
    try {
...
//---This is a middleware for security
